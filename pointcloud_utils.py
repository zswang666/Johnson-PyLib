...
def rotation_matrix_x(theta):
    """ create rotation matrix w.r.t. X axis (theta in [0,2*pi]) """
...
def rotation_matrix_y(theta):
    """ create rotation matrix w.r.t. Y axis (theta in [0,2*pi]) """
    cos, sin = tf.cos(theta), tf.sin(theta)
...
def rotation_matrix_z(theta):
    """ create rotation matrix w.r.t. Z axis (theta in [0,2*pi]) """
    cos, sin = tf.cos(theta), tf.sin(theta)
    rot = tf.stack([cos, sin, 0,
                    sin, cos, 0,
                      0,   0, 1])
    rot = tf.reshape(rot, (3,3))
    return rot
...
def tf_random_rotate_point_cloud(pc):
    """ Tensorflow op: randomly rotate a point cloud (Nx3) w.r.t. X->Y->Z axes """
    theta = tf.random_uniform([3], 0., 1.) * 2 * np.pi
    rot_x = rotation_matrix_x(theta[0])
    rot_y = rotation_matrix_y(theta[1])
    rot_z = rotation_matrix_z(theta[2])
...
def tf_jitter_point_cloud(pc, sigma, clip):
    ...
    """
    jittered_pc = tf.random_normal(pc.shape, 0, sigma)
    jittered_pc = tf.clip_by_value(jittered_pc, -clip, clip)
    return jittered_pc